...
Smits.PhyloCanvas.NewickParse.prototype = {
	...
	recursiveParse = function(clade, parentNode){
		...
		if(clade.branch_length){	// Branches can be attributes or own element
			...
			node.len = 0;//Smits.Common.roundFloat(clade.branch_length, 4);			// round to 4 decimal places
			if(node.len == 0){
				node.len = 0.0001;
			}			
		}
...
Smits.PhyloCanvas.NexmlParse.prototype = {
	...
	Rectangular : {
		bufferX			: 20, 			// Reduces the available canvas space for tree branches, allowing
...
Smits.PhyloCanvas.Render.SVG.prototype = {
	...
	draw : function(instruct){
      ...
      svgMaxWidth = Math.max(bbox.width + bbox.x, svgMaxWidth);
    ...
    scaleX -= Math.floor(0.6*scaleX); // Hack.
    scaleY -= Math.floor(0.1*scaleY); // Hack.
