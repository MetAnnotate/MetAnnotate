...
@post('/hmm-upload/')
def upload_hmm_post():
    return {'files': handle_upload(request.files.items(), verify_hmm)}
...
@post('/orf-upload/')
def upload_orf_post():
    return {'files': handle_upload(request.files.items(), verify_orf())}
    ...
    with open(path) as f:
        for l in f:
            if l.startswith('>'):
                ...
                current_len += len(l) - 1
    if 0 < consensus == current_len:
        return verify_orf(path)  # Verify Fasta format.
    else:
        return False
...
@post('/msa-upload/')
def upload_msa_post():
    return {'files': handle_upload(request.files.items(), verify_msa())}
...
def upload_tree_post():
    return {'files': handle_upload(request.files.items(), verify_tree())}
...
@post('/log-upload/')
def upload_log_post():
    return {'files': handle_upload(request.files.items(), verify_log())}


def directory_list(current):
    files = []
    for subfile in os.listdir(current):
        ...
        if os.path.isfile(full_subfile):
            ...
            sublist = directory_list(full_subfile)
            if len(sublist) > 0:
...
def metagenomes():
    ...
    with open('metagenome_directories.txt') as f:
        for l in f:
            ...
            sublist = directory_list(directory)
            if len(sublist) > 0:
...
# is_revoked is a variable later in the script. Deal with later
def IsRevoked(job):
    return job in revoked_jobs()
